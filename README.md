<h1 align="center">Hi 👋, I'm Ehzem Sheikh</h1>
<h3 align="center">
An ambitious and passionate Computer Engineering senior at Habib University with strong interests in cybersecurity, robotics, automation, and artificial intelligence.
</h3>

<p align="center">
I am driven by curiosity and a passion for exploring new horizons in technology. Eager to acquire knowledge and real-world experience, I continuously strive to embrace cutting-edge tools and systems that enhance my technical abilities. 

I’m particularly drawn to cybersecurity, robotics, automation, artificial intelligence, and data-driven systems — not just from a theoretical standpoint, but in understanding how these domains solve real life challenges. My analytical mindset and problem solving skills push me to tackle complex systems head on. 

With a goal oriented approach, strong work ethic, and the ability to thrive under pressure, I enjoy turning challenging problems into structured, practical solutions.
</p>

---

- Currently working on **Autonomous Tow Truck Project**
- Learning **ROS2, Docker, Autoware, Linux, OWASP ZAP, Burp Suite, Microsoft Threat Modeling Tool, Metasploit, Wireshark, tcpdump, and Nmap**
- Open to collaborate on **cybersecurity, robotics, and automation projects**
- Reach me at: **ehzem.sheikh@gmail.com**
- LinkedIn:  
  [www.linkedin.com/in/ehzem-farhan-sheikh-666496289](https://www.linkedin.com/in/ehzem-farhan-sheikh-666496289)

---

## Projects

### Predicting Thar Desert Expansion Using Deep Learning
- Trained spatiotemporal deep-learning models using **MODIS** and **Dynamic World satellite datasets**  
- Forecasted desertification trends in the Thar Desert  
- Supported evidence-based land-use planning and climate-resilience policy

### Throughput Analysis of OFDM in 5G New Radio (NR)
- Simulated and analyzed **5G NR PDSCH throughput**
- Evaluated varying SNR, modulation schemes, subcarrier spacing, cyclic prefix, and resource block configurations  
- Used realistic fading channel models for performance assessment

### Border Gateway Protocol (BGP) Implementation
- Implemented and simulated **BGP across multiple autonomous systems**
- Configured inter-domain routing and policy-based path selection  
- Modeled route propagation using **Cisco Packet Tracer**

### Strategic Industry Analysis of Toyota Indus Motor Company
- Conducted **PESTEL and Porter’s Five Forces analysis**
- Assessed competitive dynamics and strategic positioning in Pakistan’s automotive sector

---

## Courses

- Official **ISC2 Certified in Cybersecurity**

---

## Technical Skills

<table>
<tr>
<td valign="top" width="33%">

### Programming & Systems
- C++
- Python
- MATLAB
- RISC-V Assembly
- Verilog

</td>

<td valign="top" width="33%">

### Robotics & Embedded
- ROS
- Circuit Design
- Arduino
- Autonomous Systems

</td>

<td valign="top" width="33%">

### Tools & Platforms
- Docker
- Linux
- Qt
- MySQL

</td>

<td valign="top" width="33%">

### Cybersecurity
- Microsoft Threat Modelling Tool
- Packet Tracer

</td>
</tr>
</table>

---

<h3 align="left">Languages and Tools:</h3>
<p align="left"> 
<a href="https://www.arduino.cc/" target="_blank" rel="noreferrer">
<img src="https://cdn.worldvectorlogo.com/logos/arduino-1.svg" alt="arduino" width="40" height="40"/>
</a>
<a href="https://www.cprogramming.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/c/c-original.svg" alt="c" width="40" height="40"/>
</a>
<a href="https://www.w3schools.com/cpp/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/cplusplus/cplusplus-original.svg" alt="cplusplus" width="40" height="40"/>
</a>
<a href="https://www.docker.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg" alt="docker" width="40" height="40"/>
</a>
<a href="https://www.linux.org/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" alt="linux" width="40" height="40"/>
</a>
<a href="https://www.mathworks.com/" target="_blank" rel="noreferrer">
<img src="https://upload.wikimedia.org/wikipedia/commons/2/21/Matlab_Logo.png" alt="matlab" width="40" height="40"/>
</a>
<a href="https://www.mysql.com/" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" alt="mysql" width="40" height="40"/>
</a>
<a href="https://www.python.org" target="_blank" rel="noreferrer">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="python" width="40" height="40"/>
</a>
<a href="https://www.qt.io/" target="_blank" rel="noreferrer">
<img src="https://upload.wikimedia.org/wikipedia/commons/0/0b/Qt_logo_2016.svg" alt="qt" width="40" height="40"/>
</a>
</p>
